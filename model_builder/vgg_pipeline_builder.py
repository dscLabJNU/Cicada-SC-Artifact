...
from .vgg_pipeline_loader import VGGPipelineLoader
...
class VGGPipelineBuilder:
    """专门用于Pipeline方式加载VGG的构建器"""
    ...
    def __init__(self):
        ...
        # 构建日志文件名
        mode = os.environ.get('PIPELINE_MODE', 'sync')
    ...
    def create_pipeline_loader(self, model_name: str) -> VGGPipelineLoader:
        """创建流水线加载器"""
        if model_name not in self.pipeline_loaders:
            # 创建加载器
            self.pipeline_loaders[model_name] = VGGPipelineLoader(
                model_name=model_name
            )

        return self.pipeline_loaders[model_name]

    def _run_inference(self, layer_name: str, layer: nn.Module, current_output: torch.Tensor, loader: VGGPipelineLoader) -> Optional[torch.Tensor]:
        """执行单层推理
        ...
        Args:
            ...
            loader (VGGPipelineLoader): 加载器实例
    ...
    def load_model_pipeline_sync(self, model_name: str, weights_key: str = None, test_input: Optional[torch.Tensor] = None) -> Iterator[SyncPipelineTask]:
        """流水线方式加载模型"""
        try:
            ...
            for layer_name, layer, structure_ts_info in loader.get_layer_sequence():
                ...
                if current_output is not None:
                    current_output = self._run_inference(
                        layer_name, layer, current_output, loader)
    ...
    def load_model_pipeline_async(self, model_name: str, weights_key: Optional[str] = None,
                                  test_input: Optional[torch.Tensor] = None) -> Iterator[PipelineTask]:
        """使用并行流水线方式加载模型"""
        try:
            # 创建加载器
            loader = self.create_pipeline_loader(model_name)

            # 获取正确的层名称序列（对应create_layer生成的三个主模块）
        ...
        try:
            ...
            for layer_name, weights_key in self.pending_weights.items():
                ...
                if weights:
                    ...
                    logger.warning(f"层 {layer_name} 的权重加载失败")
                    return False
    ...
    def get_layer(self, layer_name: str) -> Optional[nn.Module]:
        ...
        for loader in self.pipeline_loaders.values():
            if hasattr(loader, 'layer_stats') and layer_name in loader.layer_stats:
                return loader.layer_stats[layer_name]
        return None

    def get_model(self, model_name: str) -> nn.Module:
        """重写父类方法，抛出异常提示使用pipeline方法"""
        raise NotImplementedError(
            "VGGPipelineBuilder 不支持直接获取完整模型，请使用 load_model_pipeline 方法进行流水线加载"
        )
