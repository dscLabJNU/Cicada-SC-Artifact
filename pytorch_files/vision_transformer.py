...
class MLPBlock(MLP):
    ...
    def __init__(self, in_dim: int, mlp_dim: int, dropout: float, model_name: str = "vit", init_weights: bool = True):
        ...
        timing_data = [
            [timestamp, model_name, f"init_weights_MLPBlock", (time.perf_counter() - t0) * 1000]
        ]
...
class EncoderBlock(nn.Module):
    ...
    def __init__(
        ...
        self.mlp = MLPBlock(hidden_dim, mlp_dim, dropout, model_name=model_name, init_weights=init_weights)
...
class Encoder(nn.Module):
    ...
    def __init__(
        ...
        self.dropout = nn.Dropout(dropout)
        timing_data.append([timestamp, model_name, "pos_embedding", (time.perf_counter() - t0) * 1000])

        # Encoder layers timing
        ...
        timing_data.append([timestamp, model_name, f"encoder_layer", (time.perf_counter() - t0) * 1000])

        # Layer norm
        t0 = time.perf_counter()
        self.ln = norm_layer(hidden_dim)
        timing_data.append([timestamp, model_name, "final_layer_norm", (time.perf_counter() - t0) * 1000])
    ...
    def __init__(
        ...
        if conv_stem_configs is not None:
            ...
            for i, conv_stem_layer_config in enumerate(conv_stem_configs):
                # Conv2dNormActivation timing
                t0 = time.perf_counter()
                conv_bn_relu = Conv2dNormActivation(
                ...
                timing_data.append([timestamp, model_name, f"conv_stem_layer_{i}", (time.perf_counter() - t0) * 1000])
                seq_proj.add_module(f"conv_bn_relu_{i}", conv_bn_relu)
                prev_channels = conv_stem_layer_config.out_channels

            # Final Conv2d timing
            t0 = time.perf_counter()
            conv_last = nn.Conv2d(in_channels=prev_channels, out_channels=hidden_dim, kernel_size=1)
            timing_data.append([timestamp, model_name, "conv_stem_final", (time.perf_counter() - t0) * 1000])
            seq_proj.add_module("conv_last", conv_last)
            self.conv_proj: nn.Module = seq_proj
        else:
            # Single Conv2d timing
            t0 = time.perf_counter()
            self.conv_proj = nn.Conv2d(
                in_channels=3, out_channels=hidden_dim, kernel_size=patch_size, stride=patch_size
            )
            timing_data.append([timestamp, model_name, "conv_proj", (time.perf_counter() - t0) * 1000])
        ...
        timing_data.append([timestamp, model_name, "heads", (time.perf_counter() - t0) * 1000])
